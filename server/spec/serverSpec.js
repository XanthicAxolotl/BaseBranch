...
describe('', function() {
  ...
  describe('Users', function(){
    ...
    after(function(done){
      ...
      .then(function(affectedRows){
        console.log('Deleted testuser from DB. Rows affected: ', affectedRows);
        db.Curricula.destroy({where: { id: curriculumId }})
        .then(function(affectedRows){
    ...
    it('should retrieve existing user', function(done){
      request(app)
        .get('/api/user/' + userId)
        .expect(200)
        .expect(function(res){
          expect(res.body.picture).to.equal('picture');
        })
        .end(done);
    });

    it('should allow user to subscribe to a curriculum', function(done){
      db.Curricula.create({
        name: 'testsubscribe',
        userId: userId
      })
      .then(function(curriculum){
        ...
        request(app)
          .post('/api/user/curricula/subscribed/' + userId)
          .send({
            userId: userId,
          ...
          .expect(200)
          .expect(function(res){
            expect(res.body.curriculaId).to.equal(curriculumId);
          })
          .end(done);
      })
    });

    it('should retrieve all subscriptions of user', function(done){
      request(app)
        .get('/api/user/curricula/subscribed/' + userId)
        .expect(200)
        .expect(function(res){
          expect(res.body[0].id).to.equal(curriculumId);
        })
        .end(done);
    });

    it('should get curricula created by user', function(done){
      request(app)
        .get('/api/user/curricula/created/' + userId)
        .expect(200)
        .expect(function(res){
          expect(res.body[0].id).to.equal(curriculumId);
        })
    ...
    it('should update new picture for user', function(done){
      request(app)
        .post('/api/user/picture/' + userId)
        .send({
          picture: 'newpicture'
        })
        .expect(200)
